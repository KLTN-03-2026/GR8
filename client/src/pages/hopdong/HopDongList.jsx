...
const HopDongList = () => {
  ...
  // Tab: 'contracts' | 'terminate-requests'
  const [activeTab, setActiveTab] = useState('contracts');
  const [terminateRequests, setTerminateRequests] = useState([]);
  ...
  const fetchTerminateRequests = async () => {
    try {
      setTrLoading(true);
      setTrError('');
      const response = await axios.get('/hopdong/terminate-requests');
      setTerminateRequests(response.data.data || []);
    } catch (err) {
      setTrError(err.response?.data?.message || 'Lỗi khi tải danh sách yêu cầu');
    } finally {
      setTrLoading(false);
    }
  };
  ...
  const handleResetTerminate = async (id) => {
    if (!window.confirm('Bạn có chắc chắn muốn hủy yêu cầu kết thúc này?')) return;
    try {
      await axios.put(`/hopdong/reset-terminate/${id}`);
      setActionMsg({ id, text: 'Đã hủy yêu cầu kết thúc', ok: true });
      setTimeout(() => {
        setActionMsg({ id: null, text: '', ok: false });
        fetchTerminateRequests();
      }, 1500);
    } catch (err) {
      setActionMsg({ id, text: err.response?.data?.message || 'Lỗi khi hủy', ok: false });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-indigo-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="w-full">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-3">
              {/* Nút Yêu cầu kết thúc hợp đồng */}
              <button
                onClick={() => {
                  setActiveTab(activeTab === 'terminate-requests' ? 'contracts' : 'terminate-requests');
                  if (activeTab !== 'terminate-requests') fetchTerminateRequests();
                }}
                className={`flex items-center px-5 py-3 rounded-lg font-semibold transition-colors shadow-md ${
                  activeTab === 'terminate-requests'
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-red-700 flex items-center gap-2">
                ...
                Danh sách yêu cầu kết thúc hợp đồng
              </h2>
              <button onClick={fetchTerminateRequests} className="text-sm text-indigo-600 hover:underline">Làm mới</button>
            </div>
            ...
            {trLoading ? (
              ...
              <div className="bg-red-50 border border-red-300 text-red-700 px-4 py-3 rounded-lg">{trError}</div>
            ) : terminateRequests.length === 0 ? (
              <div className="bg-white rounded-xl shadow-md p-12 text-center">
                <div className="text-5xl mb-3">📄</div>
                <h3 className="text-lg font-semibold text-gray-700">Không có yêu cầu nào</h3>
                <p className="text-gray-500 text-sm mt-1">Chưa có cư dân nào gửi yêu cầu kết thúc hợp đồng</p>
              </div>
            ) : (
              <div className="grid gap-4">
                {terminateRequests.map(contract => (
                  <div key={contract.ID} className="bg-white rounded-xl shadow-md border-l-4 border-red-500 p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-lg font-bold text-gray-900">Hợp đồng #{contract.ID}</h3>
                          <span className="px-2.5 py-0.5 bg-red-100 text-red-700 text-xs font-bold rounded-full border border-red-300">
                            ⚠️ Yêu cầu kết thúc
                          </span>
                        </div>
                        <div className="grid md:grid-cols-2 gap-x-8 gap-y-1 text-sm text-gray-600 mb-3">
                          <p><span className="font-medium">Căn hộ:</span> {contract.canho?.MaCanHo} - Phòng {contract.canho?.SoPhong}</p>
                          <p><span className="font-medium">Người thuê:</span> {contract.nguoidung?.HoTen}</p>
                          <p><span className="font-medium">Email:</span> {contract.nguoidung?.Email}</p>
                          <p><span className="font-medium">SĐT:</span> {contract.nguoidung?.SoDienThoai || '—'}</p>
                          <p><span className="font-medium">Ngày bắt đầu:</span> {new Date(contract.NgayBatDau).toLocaleDateString('vi-VN')}</p>
                          <p><span className="font-medium">Ngày kết thúc:</span> {new Date(contract.NgayKetThuc).toLocaleDateString('vi-VN')}</p>
                          {contract.NgayYeuCauKetThuc && (
                            <p><span className="font-medium">Ngày yêu cầu:</span> {new Date(contract.NgayYeuCauKetThuc).toLocaleDateString('vi-VN')}</p>
                          )}
                        </div>
                        {contract.LyDoKetThuc && (
                          <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-2.5 text-sm">
                            <span className="font-medium text-red-700">Lý do: </span>
                            <span className="text-gray-700">{contract.LyDoKetThuc}</span>
                          </div>
                        )}
                        {actionMsg.id === contract.ID && (
                          <p className={`mt-2 text-sm font-medium ${
                            actionMsg.ok ? 'text-green-600' : 'text-red-600'
                          }`}>{actionMsg.text}</p>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col gap-2 flex-shrink-0">
                        <button
                          ...
                          className="px-4 py-2 bg-red-600 text-white text-sm font-semibold rounded-lg hover:bg-red-700 transition-colors shadow"
                        >
                          ✔ Duyệt kết thúc
                        </button>
                        <button
                          onClick={() => handleResetTerminate(contract.ID)}
                          className="px-4 py-2 bg-gray-100 text-gray-700 text-sm font-semibold rounded-lg hover:bg-gray-200 transition-colors border border-gray-300"
                        >
                          ✕ Hủy yêu cầu
                        </button>
                        <button
                          onClick={() => window.location.href = `/hopdong/${contract.ID}`}
                          className="px-4 py-2 bg-indigo-50 text-indigo-700 text-sm font-semibold rounded-lg hover:bg-indigo-100 transition-colors border border-indigo-200"
                        >
                          👁 Xem chi tiết
                        </button>
                      </div>
        ...
        {/* ==== PANEL: Danh sách hợp đồng (tab mặc định) ==== */}
        {activeTab === 'contracts' && (
          <>
          {contracts.length === 0 ? (
          <div className="bg-white rounded-xl shadow-md p-12 text-center">
