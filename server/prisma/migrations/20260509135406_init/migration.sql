...
CREATE TABLE `canho` (
    ...
    `MoTa` TEXT NULL,
    `IsHot` BOOLEAN NULL DEFAULT false,
    `GioiHanNguoiO` INTEGER NULL DEFAULT 2,
    `ChuNhaID` INTEGER NOT NULL,
    `updated_at` DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0),
    `is_deleted` TINYINT NULL DEFAULT 0,
    `deleted_at` DATETIME(0) NULL,
    `NgayTinhDien` TINYINT UNSIGNED NULL,
    `NgayTinhNuoc` TINYINT UNSIGNED NULL,
    `NgayTinhTienNha` TINYINT UNSIGNED NULL,
...
CREATE TABLE `chisodiennuoc` (
    ...
    `ChiSoDienMoi` DECIMAL(10, 2) NULL,
    `NgayGhiDien` DATETIME(0) NULL,
    `ChiSoNuocCu` DECIMAL(10, 2) NULL DEFAULT 0.00,
    `ChiSoNuocMoi` DECIMAL(10, 2) NULL,
    `NgayGhiNuoc` DATETIME(0) NULL,
    `NgayGhi` DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0),
    `NguoiGhiID` INTEGER NULL,
    `thang_nam_date` DATE NULL,
    `AnhDongHoDien` VARCHAR(500) NULL,
    `AnhDongHoNuoc` VARCHAR(500) NULL,
    `ChiSoDienChinhThuc` DECIMAL(10, 2) NULL,
    `ChiSoNuocChinhThuc` DECIMAL(10, 2) NULL,
    `GhiChuKeToan` TEXT NULL,
    `KeToanDuyetID` INTEGER NULL,
    `NgayKeToanDuyet` DATETIME(0) NULL,
    `TrangThai` ENUM('ChoDienHoacNuoc', 'ChoDuyetKeToan', 'DaDuyetChoPhatHanh', 'DaDuyet', 'DaPhatHanhHoaDon', 'TuChoi') NULL DEFAULT 'ChoDuyetKeToan',

    INDEX `chisodiennuoc_CanHoID_idx`(`CanHoID`),
    INDEX `chisodiennuoc_NguoiGhiID_idx`(`NguoiGhiID`),
    INDEX `chisodiennuoc_TrangThai_idx`(`TrangThai`),
...
CREATE TABLE `chuyennhuong` (
    ...
    `NguoiThueMoiID` INTEGER NULL,
    `ThongTinNguoiChuyenVao` TEXT NULL,
    `LyDo` TEXT NULL,
    `GhiChu` TEXT NULL,
    ...
    `NgayHen` DATETIME(0) NULL,
    `NoiDungHen` TEXT NULL,
...
CREATE TABLE `hopdong` (
    ...
    `YeuCauKetThuc` BOOLEAN NULL DEFAULT false,
    `LyDoKetThuc` TEXT NULL,
    `NgayYeuCauKetThuc` DATETIME(0) NULL,
    `updated_at` DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0),
    `is_deleted` TINYINT NULL DEFAULT 0,
    `deleted_at` DATETIME(0) NULL,
...
CREATE TABLE `nguoidung` (
    ...
    `DaKhaiBaoNgoaiTru` BOOLEAN NULL DEFAULT false,
...
CREATE TABLE `yeucaudichvu` (
    ...
    INDEX `idx_yeucaudichvu_hoadon`(`HoaDonID`),
    PRIMARY KEY (`ID`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `yeucauthue` (
    ...
    `CanHoID` INTEGER NOT NULL,
    `NgayYeuCau` DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0),
    `TrangThai` ENUM('ChoKiemTra', 'DatLich', 'DaDuyet', 'TuChoi') NULL DEFAULT 'ChoKiemTra',
    `GhiChu` TEXT NULL,
    `QuanLyKiemTraID` INTEGER NULL,
    `ChuNhaDuyetID` INTEGER NULL,
    `NgayXemDuKien` DATETIME(0) NULL,
...
CREATE TABLE `thanh_vien_canho` (
    ...
    `NgayRoi` DATETIME(0) NULL,
    `TrangThai` ENUM('DangO', 'DaRoi') NULL DEFAULT 'DangO',
    `GhiChu` TEXT NULL,
    `NguoiThemID` INTEGER NULL,

    INDEX `thanh_vien_canho_CanHoID_idx`(`CanHoID`),
    INDEX `thanh_vien_canho_NguoiThemID_idx`(`NguoiThemID`),
    PRIMARY KEY (`ID`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `lichtruc` (
    ...
    `CaTruc` ENUM('Sang', 'Chieu', 'Toi', 'CaNgay') NOT NULL DEFAULT 'CaNgay',
    `GhiChu` TEXT NULL,
    `TrangThai` ENUM('DangTruc', 'DaTruc', 'Huy') NULL DEFAULT 'DangTruc',
    `NguoiTaoID` INTEGER NULL,
    `NgayTao` DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0),
    `updated_at` DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0),

    INDEX `lichtruc_NhanVienID_idx`(`NhanVienID`),
    INDEX `lichtruc_NguoiTaoID_idx`(`NguoiTaoID`),
    INDEX `idx_lichtruc_ngaytruc`(`NgayTruc`),
    INDEX `idx_lichtruc_nhanvien_ngay`(`NhanVienID`, `NgayTruc`),
    PRIMARY KEY (`ID`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `yeucaudichvu` ADD CONSTRAINT `fk_yeucaudichvu_hoadon` FOREIGN KEY (`HoaDonID`) REFERENCES `hoadon`(`ID`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `thanh_vien_canho` ADD CONSTRAINT `thanh_vien_canho_CanHoID_fkey` FOREIGN KEY (`CanHoID`) REFERENCES `canho`(`ID`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `thanh_vien_canho` ADD CONSTRAINT `thanh_vien_canho_NguoiThemID_fkey` FOREIGN KEY (`NguoiThemID`) REFERENCES `nguoidung`(`ID`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `lichtruc` ADD CONSTRAINT `lichtruc_NguoiTaoID_fkey` FOREIGN KEY (`NguoiTaoID`) REFERENCES `nguoidung`(`ID`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `lichtruc` ADD CONSTRAINT `lichtruc_NhanVienID_fkey` FOREIGN KEY (`NhanVienID`) REFERENCES `nguoidung`(`ID`) ON DELETE CASCADE ON UPDATE CASCADE;
